...
public class SkillSkill : ISkill
{
    ...
    public ushort SkillNeedPoints { get; set; }//技能所需的技能点

    public bool TryUseSkill(ushort ChrHasPoints) //尝试使用技能
    {
        if (ChrHasPoints < SkillNeedPoints)
        {
            return false;
        }
        ...
        )
    {
        ...
        sputteringType = _sputteringType;
        SkillNeedPoints = _SkillNeedPoints;
    ...
    private void ReducePoints(BSBaseData _skillSource)
    {
        if (_skillSource != null)
        {
            _skillSource.skillPoints -= SkillNeedPoints;
        }
    }
    ...
    private void BuffSource(BSBaseData _skillSource, SkillEventBag skillEventBag)
    {
        ...
        if (_skillSource != null
            && BuffLibrary.Instance.buffLibrary[skillEventBag.BuffID] != null)
        {
            _skillSource.buffComponent.AddBuff(
                BuffLibrary.Instance.buffLibrary[skillEventBag.BuffID].
                ModifyBuffValue(skillEventBag.GetEventModifyBuffValue(skillLevel)));
    ...
    public void AddEventFromBag()
    {
        ...
        {
            var eventBag = i;
            //根据技能事件包列表中的配置，动态构建技能的事件处理委托链
            switch (i.skillEventType)
            {
                case SkillEventType.ConsumeResources://消耗资源的事件的添加
                    SkillEvent += (sender, e) => ReducePoints(skillSource);
                    break;
    ...
    public SkillSkill AddEventBagToList(
    ...
    {
        skillEventBags.Add(new SkillEventBag(skillEventType, statType, 
            EventModifyBuffValue, EventBaseValue, EventBaseValueFunc, BuffID));
        return this;
    }
